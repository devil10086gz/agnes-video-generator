...
from core.image_generator import ImageGeneratorAgnesAPI
from core.video_generator import VideoGeneratorAgnesAPI
...
class VideoPipeline:
    def __init__(
        ...
        self.video_generator = VideoGeneratorAgnesAPI(api_key=api_key, model=video_model)
        self.video_generator.shutdown_event = shutdown_event

        self._state: Optional[TaskState] = None

    async def _emit(self, step: str, status: str, message: str, progress: float = 0.0, data: dict = None):
    ...
    async def _step_image_analysis(self, reference_image: str, end_frame_images: list) -> str:
        if self._state.step_image_analysis == StepStatus.COMPLETED:
            analysis_file = self._state.image_analysis_file
            if os.path.exists(analysis_file):
                logger.info(f"[Pipeline] Step image_analysis: SKIP (already completed, file exists)")
                with open(analysis_file, "r") as f:
                    return f.read()
            logger.warning(f"[Pipeline] Step image_analysis: marked completed but file missing, re-running")
            return ""

        logger.info(f"[Pipeline] Step image_analysis: RUNNING")
        images_to_analyze = []
        if reference_image:
    ...
    async def _step_story(self, image_context: str) -> str:
        if self._state.step_story == StepStatus.COMPLETED:
            ...
            if os.path.exists(story_path):
                logger.info(f"[Pipeline] Step story: SKIP (already completed, file exists)")
                with open(story_path, "r") as f:
                    return f.read()
            logger.warning(f"[Pipeline] Step story: marked completed but file missing, re-running")

        logger.info(f"[Pipeline] Step story: RUNNING")
    ...
    async def _step_character_reference(self, story: str) -> str:
        if self._state.step_character_ref == StepStatus.COMPLETED:
            ...
            if ref_path and os.path.exists(ref_path):
                logger.info(f"[Pipeline] Step character_ref: SKIP (already completed, file exists)")
                return ref_path
            logger.warning(f"[Pipeline] Step character_ref: marked completed but file missing, re-running")

        if self._state.reference_image:
            logger.info(f"[Pipeline] Step character_ref: SKIP (user-provided reference image)")
    ...
    async def _step_script(self, story: str) -> list:
        if self._state.step_script == StepStatus.COMPLETED:
            ...
            if os.path.exists(script_path):
                logger.info(f"[Pipeline] Step script: SKIP (already completed, file exists)")
                with open(script_path, "r") as f:
                    scenes = json.load(f)
                if self._state.scene_count == len(scenes):
                    return scenes
                logger.warning(f"[Pipeline] Step script: scene count mismatch, re-running")
            else:
                logger.warning(f"[Pipeline] Step script: marked completed but file missing, re-running")

        logger.info(f"[Pipeline] Step script: RUNNING")
    ...
    async def _step_end_frame_prompts(self, story: str, scenes: list) -> list:
        ...
        if self._state.step_end_frame_prompts == StepStatus.COMPLETED:
            ...
            if os.path.exists(prompts_path):
                logger.info(f"[Pipeline] Step end_frame_prompts: SKIP (already completed, file exists)")
                with open(prompts_path, "r") as f:
                    return json.load(f)
            logger.warning(f"[Pipeline] Step end_frame_prompts: marked completed but file missing, re-running")

        logger.info(f"[Pipeline] Step end_frame_prompts: RUNNING")
    ...
    async def _step_pregenerate_end_frames(
        ...
        for scene_idx in range(len(scenes)):
            ...
            if user_ef:
                await self._emit("end_frame_gen", "running", f"场景 {scene_idx+1}/{len(scenes)}: 使用自定义尾帧", 0.25 + 0.05 * scene_idx / len(scenes))
            ...
            if self._state.generate_end_frames_from_ref and character_ref_path:
                ...
                end_frame_prompt = end_frame_prompts[scene_idx] if scene_idx < len(end_frame_prompts) else "cinematic end frame"
                await self._emit("end_frame_gen", "running", f"场景 {scene_idx+1}/{len(scenes)}: 自动生成尾帧 (t2i)", 0.25 + 0.05 * scene_idx / len(scenes))
                img_output = await self.image_generator.generate_single_image(
                    prompt=end_frame_prompt,
                    size=f"{vw}x{vh}",
                )
        ...
        await self._emit("end_frame_gen", "completed", f"尾帧预生成全部完成 ({len(pregenerated)}/{len(scenes)})", 0.35)
        return pregenerated
    ...
    def _save_scene_task(self, scene_dir: str, video_id: str):
    ...
    def _load_scene_task(self, scene_dir: str):
        task_file = os.path.join(scene_dir, "task.json")
    ...
    async def _step_generate_videos(self, scenes: list, character_ref_path: str, end_frame_prompts: list, pregenerated_end_frames: dict) -> list:
        ...
        if chaining_mode == "keyframes":
            all_video_paths = await self._generate_keyframe_scenes(
                scenes, character_ref_path, end_frame_prompts, pregenerated_end_frames, vw, vh, end_frame_images
            )
        elif chaining_mode == "ti2vid":
            all_video_paths = await self._generate_chained_scenes(scenes, character_ref_path, vw, vh)
        else:
            all_video_paths = await self._generate_independent_scenes(scenes, character_ref_path, vw, vh)
    ...
    async def _generate_chained_scenes(self, scenes: list, reference_image: str, vw: int, vh: int) -> list:
        all_video_paths = []
    ...
    async def _generate_keyframe_scenes(
        ...
        for info in new_submissions:
            scene_idx = info["scene_idx"]
            await self._emit("video_gen", "running", f"场景 {scene_idx+1}/{total}: 提交任务...", 0.35 + 0.05 * scene_idx / total)
            video_id = await self.video_generator.submit_video(
        ...
        if pending:
            await self._emit("video_gen", "running", f"等待 {len(pending)} 个视频生成完成...", 0.4)

        for info in pending:
            scene_idx = info["scene_idx"]
            await self._emit("video_gen", "running", f"场景 {scene_idx+1}/{total}: 等待生成中...", 0.4 + 0.4 * pending.index(info) / len(pending))
            try:
                video_output = await self.video_generator.wait_for_video(info["video_id"])
                video_output.save(info["video_path"])
                await self._emit("video_gen", "running", f"场景 {scene_idx+1}/{total}: 完成", 0.4 + 0.4 * (pending.index(info) + 1) / len(pending))
            except Exception as e:
                logger.error(f"Scene {scene_idx} video failed: {e}")
                task_file = os.path.join(info["scene_dir"], "task.json")
                if os.path.exists(task_file):
    ...
    async def _step_concatenate(self, all_video_paths: list) -> str:
        ...
        await self._emit("concatenate", "running", "正在拼接视频...", 0.85)
    ...
    async def run(self, state: TaskState) -> str:
        self._state = state
        ...
        try:
            ...
            await self._emit("done", "completed", "视频生成完成!", 1.0, {"final_video": final_video_path})
